...
static void add(char* dirname)
{
     ...
     * 
     */
    if ( (dirname[len -1] == '.' && (dirname[len-2] == '/' || dirname[len-2] == '.' && dirname[len-3] == '/'))) return;
    if (strcmp(dirname, "/dev/fd") == 0) return 0;
    if ((ddp = malloc(sizeof(struct devdir))) == NULL) return;
...
static char* searchdir(char* dirname, struct stat* fdstatp)
{
    ...
    while ((dirp = readdir(dp) != NULL)){
        ...
        /*
            Skip aliases.
        */
       if (strcmp(pathname, "/dev/stdin") == 0 || strcmp(pathname, "/dev/stdout") == 0 || strcmp(pathname, "/dev/stderr") == 0) continue;
       if (stat(pathname, &devstat) < 0) continue;
...
char* ttyname(int fd)
{
    ...
    if (rval == NULL)
    {
        ...
        {
            if ((rval = searchdir(ddp->d_name, &fdstat)) != NULL) break;
        }
    }
    cleanup();
    return(rval);
